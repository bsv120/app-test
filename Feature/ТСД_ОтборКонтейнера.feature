...
И я делаю "ЧислоВыполнений" раз
	Попытка
		...
		Иначе	
			...
			Если существует переменная "_НомерТСД" Тогда
				Тогда в поле с именем 'Сообщение' я ввожу текст 'Автобот. Отбор контейнера;{_НомерТСД}'
			Иначе	
				Тогда в поле с именем 'Сообщение' я ввожу текст 'Автобот. Отбор контейнера;{НомерТСД}'
