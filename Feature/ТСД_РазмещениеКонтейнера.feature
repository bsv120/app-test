...
И я делаю "ЧислоВыполнений" раз
	Попытка
		...
		Если существует переменная "_НомерТСД" Тогда
			Тогда в поле с именем 'Сообщение' я ввожу текст 'Автобот. Размещение контейнера;{_НомерТСД}'
		Иначе	
			Тогда в поле с именем 'Сообщение' я ввожу текст 'Автобот. Размещение контейнера;{НомерТСД}'
