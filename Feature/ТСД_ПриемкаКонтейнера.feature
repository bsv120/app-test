...
Попытка
	...
	И я делаю "ЧислоСтрокЗаказа" раз
		...
		Если существует переменная "_НомерТСД" Тогда
			Тогда в поле с именем 'Сообщение' я ввожу текст 'Автобот. Проведение строки приемки;{_НомерТСД}'
		Иначе	
			Тогда в поле с именем 'Сообщение' я ввожу текст 'Автобот. Проведение строки приемки;{НомерТСД}'
